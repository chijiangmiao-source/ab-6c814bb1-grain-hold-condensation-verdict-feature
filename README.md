...
同样不参与减法复算。靠港前需要集中抄录多舱测量时，录入页可切换到**批量模式**，
最多二十行按测量先后一次提交，服务端在单个事务内逐行保存、批内同舱依次成链，
任一行非法则整批回滚（详见下文“批量抄录”）。
...
| POST | `/api/assessments/batch` | 靠港前批量抄录：有序测量数组（1–20 行）一次提交，单事务保存 |
...
大副在录入页可切换到**批量模式**，按测量先后一次填写最多 **20 行**航次、舱号、
粮温、气温与湿度并整批提交：
...
- 服务端接收**有序**测量数组，逐行复用与单条 `POST /api/assessments` 完全相同的
  结构校验、字段解析、`decision.Evaluate` 范围校验与露点判定；浏览器同样不参与复算。
- 全部行合法时，在**单个数据库事务**内依次保存。前序关联逐行在该事务内解析：
  - 同航次同舱在**本批较早位置**已出现时，后续行关联本批较早记录（例如测量顺序
    `1H, 2P, 1H, 3H, 1H` 中第三个 `1H` 关联第一个、第五个关联第三个）；
  - 本批未出现过的舱位，仍各自承接数据库中该航该舱按 id 的最近有效前序；
  - 该航该舱的首测无前序。
- **任一行非法（422）整批不落库**：响应同时给出分组的 `rows`（含 1 基 `row` 行号与
  该行原始 `fields` 字段错误）和扁平的 `fields`（每项重复 `row`），页面保留全部输入、
  高亮并定位到首个问题行；保存过程中失败（500）也在同一事务中回滚，不留部分记录、
  不留断裂的 `prev_id` 关联。
- 成功响应为 `201 {"items":[…]}`，数组按提交顺序排列，每行与单条 POST 成功体
  **同形**（含 `formula`、不含 `comparison`），页面逐行展示评估编号、未舍入/展示温差
  与结论，并可进入既有详情页；历史列表与舱位概览立即按真实创建顺序（id）反映这些记录。
- 文档形态错误（顶层不是对象、缺少/非数组 `items`、数组元素不是对象、未知顶层字段、
  行内字段重复声明、对象后多余内容等）一律 **400**；空数组或超过 20 行也是 **400**，
  均不落库。
...
```json
{
  ...
  "fields": [
    { "row": 2, "field": "tg", "code": "out_of_range", "message": "…" }
  ]
}
```
